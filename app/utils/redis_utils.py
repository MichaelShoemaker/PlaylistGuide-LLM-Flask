...
def check_redis(question):
    ...
    stored_questions = redis_client.hgetall("questions")  # {question: embedding_json}
    ...
    for stored_question, embedding_json in stored_questions.items():
        ...
        if similarity > 0.85:
            print(f"Found similar question in Redis: {stored_question} (Similarity: {similarity})")
            return redis_client.get(f"answer:{stored_question}")  # Return cached answer
    ...
    try:
        ...
        redis_client.hset("questions", question, json.dumps(current_embedding.tolist()))
